...
export default function LeaveRequest({ user, language = 'en' }) {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const data = await leavesApi.getByEmail(user.email || '');
      if (data) {
        const remote = data.map(l => ({
          ...
          days: l.days || 1,
          reason: l.reason || '',
          status: String(l.status || 'Pending').trim(),
          partial: l.partial || false,
          fromTime: l.from_time || null,
          toTime: l.to_time || null,
          auditNote: l.audit_note || null,
  ...
  const doSubmit = async () => {
    ...
    try {
      await leavesApi.create({
        id: tempId,
        staffName: targetName,
        staffEmail: mode === 'staff' ? (selectedStaff?.email || '') : user.email,
        type: form.type,
        from: form.from,
        to: form.partial ? form.from : (form.to || form.from),
        days: duration,
        ...
        status: 'Pending',
        partial: form.partial,
        fromTime: form.partial ? form.fromTime : '',
        toTime: form.partial ? form.toTime : '',
        emergency: form.emergency,
      });
    } catch {}
