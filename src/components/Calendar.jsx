...
import { calendarApi, settingsApi } from '../lib/supabaseApi';
...
export default function Calendar({ user, language }) {
  ...
  const [scopeOverride, setScopeOverride] = useState(defaultScope);
  const [countryOverride, setCountryOverride] = useState(meta.country || '');
  const allCountries = Object.keys(COUNTRY_CENTRES);

  useEffect(() => {
    setScopeOverride(defaultScope);
    setCountryOverride(meta.country || '');
  }, [defaultScope, meta.country]);
  ...
  const normalizeEvents = (data) => data.map(r => ({
    id: r.id,
    date: (r.event_date || '').split('T')[0],
    name: r.name,
    type: r.type || 'event',
    desc: r.description || '',
  ...
  const fetchSheetEvents = useCallback(async (targetYear, targetMonth) => {
    setSyncStatus('syncing');
    try {
      const data = await calendarApi.getByMonth(targetYear, targetMonth);
  ...
  const handleAddEvent = async (e) => {
    ...
    const scope = meta.isSuperAdmin ? scopeOverride : getDefaultScope(meta);
    const eventCountry = scope === 'global'
      ? ''
      : (meta.isSuperAdmin && (scope === 'national' || scope === 'centre'))
        ? countryOverride
        : (meta.country || '');
    if ((scope === 'national' || scope === 'centre') && !eventCountry) {
      setSyncStatus('fail');
      return;
    }
    ...
    try {
      const ok = await calendarApi.create(ev);
      if (ok === false) throw new Error('Insert failed');
      await fetchSheetEvents(year, month);
      setSyncStatus('done');
      setShowDayAddForm(false);
  ...
  const saveEdit = async () => {
    ...
    if (targetEvent.source === 'local') {
      ...
      setSyncStatus('syncing');
      try {
        await calendarApi.update(editingEventId, editForm);
  ...
  const handleBulkAdd = async (e) => {
    ...
    if (newEvents.length > 0) {
      try { await calendarApi.createMany(newEvents); } catch {}
    }
  ...
  const handleSatToggle = async (checked) => {
    try {
      const success = await settingsApi.upsert(meta.centre || '', 'showSat', checked);
      if (success !== false) {
        setShowSat(checked);
        try { localStorage.setItem(`haazimi_show_sat_${meta.centre || 'default'}`, String(checked)); } catch {}
      }
    } catch {
      // Keep existing state if DB fails
    }
  };
      ...
      <div className="calendar-layout">
            ...
            <div style={{
              ...
              {showDayAddForm && (
                <form onSubmit={handleAddEvent} style={{ padding: '14px 16px', borderBottom: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {meta.canWriteSheet && !meta.isSuperAdmin && (
                  ...
                  {meta.isSuperAdmin && (
                    <div style={{ display: 'grid', gridTemplateColumns: scopeOverride === 'global' ? '1fr' : '1fr 1fr', gap: 10 }}>
                      <div>
                        <label style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>{t.scopeLabel}</label>
                        <select
                          style={inputStyle}
                          value={scopeOverride}
                          onChange={e => setScopeOverride(e.target.value)}
                        >
                          <option value="global">{t.globalEvent}</option>
                          <option value="national">{t.nationalEvent}</option>
                        </select>
                      </div>
                      {scopeOverride === 'national' && (
                        <div>
                          <label style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>{t.country || 'Country'} *</label>
                          <select
                            style={inputStyle}
        ...
        <div className="calendar-top-info">
          ...
          <div className="upcoming-events">
            <h3>{MONTHS_BY_LANG[language]?.[month] || MONTHS_BY_LANG.en[month]} — {t.allEvents}</h3>
            <ul>
