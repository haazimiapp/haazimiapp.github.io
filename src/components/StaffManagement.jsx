...
import { centreStaffApi } from '../lib/supabaseApi';
...
export default function StaffManagement({ user, isDev: isDevProp, language }) {
  ...
  const loadStaff = async () => {
    ...
    setFetchError('');
    try {
      const rows = await centreStaffApi.getAll();
      if (rows) {
        setStaff(rows);
      } else {
  ...
  const handleAdd = async (form) => {
    ...
    setShowModal(false);
    try { await centreStaffApi.create(newMember); } catch {}
    setSaving(false);
  };
  ...
  const handleEdit = async (form) => {
    ...
    setShowModal(false);
    setEditMember(null);
    try { await centreStaffApi.update(editMember.id, form); } catch {}
    setSaving(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try { await centreStaffApi.delete(id); } catch {}
    setSaving(false);
  };
  ...
  const handleStatusChange = async (member, newStatus) => {
    ...
    setOpenMenu(null);
    try { await centreStaffApi.update(member.id, { status: newStatus }); } catch {}
  };
