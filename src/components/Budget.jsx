...
import { budgetApi } from '../lib/supabaseApi';
...
export default function Budget({ user, isDev: isDevProp, language }) {
  ...
  const loadBudget = async () => {
    ...
    setFetchError('');
    try {
      const rows = await budgetApi.getByMonth(mk);
  ...
  const handleAdd = async (form) => {
    ...
    setShowModal(false);
    try {
      await budgetApi.create({ ...newItem, addedBy: user?.name || '', centre: user?.centre || '', country: user?.country || '' });
    } catch {}
    setSaving(false);
  };
  ...
  const handleEdit = async (form) => {
    ...
    setShowModal(false);
    setEditItem(null);
    try { await budgetApi.update(editItem.id, form); } catch {}
    setSaving(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try { await budgetApi.delete(id); } catch {}
    setSaving(false);
  };
